...
const Modal = ({ style, children, node }: IComponentProps) => {
  return (
    <div style={{ ...style, ...node.props.node }} className={styles.modalMask + ' ' + node.className}>
      {children}
    </div>
  );
};
...
Modal.initManifest = (component: IComponentInstance) => {
  set(component.children[0].children[0], 'events.click.keyPath', `$modals.${component.id}`);
  set(component, 'pattern.display', 'function() { return ${{$modals.' + component.id + '}}; }');
  return component;
};
