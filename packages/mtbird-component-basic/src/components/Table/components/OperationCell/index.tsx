...
export const OperationCell = <RecordType extends BaseRecordType>({ editingKey, record, save, edit, omit }: IOperationCellProps<RecordType>) => {
  ...
  return isEditing ? (
    ...
    <span>
      <Button type="text" disabled={!canModify} onClick={() => edit(record)} style={{ marginRight: 8 }}>
        <i className="mtbird-icon mtbird-edit-square" />
      </Button>
      <Button type="text" disabled={!canModify} onClick={() => omit(record.key)}>
