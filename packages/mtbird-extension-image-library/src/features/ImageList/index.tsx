...
const ImageList = ({ list, context }: IProps) => {
  ...
  const generateContent = (cur: IImage) => (
    <>
      <Image src={cur.urls.raw} width={cur.width / 10} height={cur.height / 10} onClick={() => handleClick(cur)} preview={false} />
    </>
  );
  ...
  return (
    <div className={styles.imageListWrapper}>
      <div className={styles.imageListContainer}>
        {list.map((cur, i) => (
          <Popover key={cur.id} placement="topLeft" title={cur.description} content={generateContent(cur)}>
            <Image
