...
const ShapePanel = () => {
  ...
  return (
    <div className={style.shapePanelContaienr}>
      {utils.SHAPE_LIST.map((cur: { type: string; children: any[] }) => {
        return (
          <div className={style.shapePanelRow}>
            <div className={style.shapePanelTitle}>{cur.type}</div>
            <div className={style.shapePanelContent}>
              {cur.children.map((shape: any) => {
                const node = utils.generateShape(shape, 28);
                return (
                  <div className={style.shape} onClick={() => handleAddShape(shape)}>
