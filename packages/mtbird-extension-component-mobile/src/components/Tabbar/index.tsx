...
const Tabbar = ({ node, style }: IComponentProps) => {
  ...
  return (
    <div className={styles.tabbarWrapper} style={{ ...style, borderTop, border: 'unset' }}>
      {options.map((cur: Record<string, any>) => {
        return (
          <div
            className={styles.tabbarItem + ' ' + (cur.isActive ? styles.tabbarItemActive : '')}
            style={{ color: style.color, fontSize: style.fontSize }}
            onClick={() => cur.href && (location.href = cur.href)}
          >
            {showIcon && (!cur.iconType || cur.iconType === 'icon') && <i className={styles.tabbarIcon + ` mtbird-icon ${cur.icon}`} />}
            {showIcon && cur.iconType === 'image' && <img className={styles.tabbarIconImage} src={cur.icon} />}
