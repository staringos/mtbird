...
const Navbar = ({ node, style }: IComponentProps) => {
  ...
  return (
    <div className={styles.navbarWrapper} style={{ ...style, borderBottom: style.border, border: 'unset' }}>
      <div className={styles.headerBackButton}>
        {hasBackButton && <i className="mtbird-icon mtbird-left" onClick={() => backHref && (location.href = backHref)} />}
      </div>
